...
async function processCombatEventWrapper(event: GameEvent): Promise<EventResult> {
  ...
  // Если успешно - обновляем состояние в TruthSystem
  if (result.success && result.changes?.character) {
    const changes = result.changes.character as CharacterChanges;

    // Обновляем Ци
    if (changes.currentQi !== undefined) {
      const character = session.character;
      const qiDiff = changes.currentQi - character.currentQi;
      
      if (qiDiff < 0) {
        TruthSystem.spendQi(event.sessionId, Math.abs(qiDiff));
      } else if (qiDiff > 0) {
        TruthSystem.addQi(event.sessionId, qiDiff);
      }
    }
