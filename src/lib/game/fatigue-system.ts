...
/**
 * Расчёт усталости от потраченной Ци
 * Используется в combat-processor для расчёта усталости при использовании техник
 */
export function calculateFatigueFromQiSpent(
  qiSpent: number,
  cultivationLevel: number
): { physicalFatigue: number; mentalFatigue: number } {
  // Получаем множитель накопления усталости
  const accumulationMultiplier = getFatigueAccumulationMultiplier(cultivationLevel);
  
  // Физическая усталость: минимальная, 0.005% за 100 Ци
  const physicalFatigue = qiSpent * 0.005 * accumulationMultiplier;
  
  // Ментальная усталость: 1% за 100 Ци
  const mentalFatigue = qiSpent * 0.01 * accumulationMultiplier;
  
  return {
    physicalFatigue: Math.min(physicalFatigue, 100),
    mentalFatigue: Math.min(mentalFatigue, 100),
  };
}
