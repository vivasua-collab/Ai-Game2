...
export async function GET(request: NextRequest) {
  try {
    ...
    // Генерируем уникальный ID с счётчиком для избежания дубликатов
    let bufferCounter = 0;
    const allLogs: LogEntry[] = [...bufferLogs.map(log => ({
      id: log.id || `buffer-${Date.now()}-${++bufferCounter}`,
    ...
    return NextResponse.json({
      ...
      qiLogs: {
        enabled: isQiLoggingEnabled(),
        logs: getQiLogBuffer().slice(-50).map(log => ({
          timestamp: log.timestamp.toISOString(),
          sessionId: log.sessionId,
          characterId: log.characterId,
          oldQi: log.oldQi,
          newQi: log.newQi,
          qiChange: log.qiChange,
          source: log.source,
          reason: log.reason,
          details: log.details,
        })),
