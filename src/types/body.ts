...
// ==================== СИСТЕМА РАЗМЕРОВ ТЕЛА ====================

/**
 * Класс размера тела
 * Влияет на дальность техник и силу атак
 */
export type BodySizeClass = 'tiny' | 'small' | 'medium' | 'large' | 'huge' | 'gargantuan' | 'colossal';

/**
 * Конфигурация класса размера тела
 */
export interface BodySizeConfig {
  ...
  /** Множитель дальности техник */
  rangeMultiplier: number;
}

/**
 * Конфигурации классов размера тела
 */
export const BODY_SIZE_CONFIGS: Record<BodySizeClass, BodySizeConfig> = {
  tiny: {
    minHeight: 0,
    maxHeight: 30,
...
/**
 * Определяет класс размера тела по высоте
 *
 * @param heightCm - Рост в сантиметрах
 * @returns Класс размера тела
 */
...
/**
 * Получает конфигурацию размера тела
 *
 * @param sizeClass - Класс размера тела
 * @returns Конфигурация размера
 */
export function getBodySizeConfig(sizeClass: BodySizeClass): BodySizeConfig {
  return BODY_SIZE_CONFIGS[sizeClass];
}

/**
 * Рассчитывает эффективную дальность техники с учётом размера тела
 *
 * @param baseRange - Базовая дальность из техники (метры)
 * @param bodyHeightCm - Рост атакующего (см)
 * @returns Эффективная дальность (метры)
 */
export function calculateEffectiveRange(baseRange: number, bodyHeightCm?: number): number {
  if (!bodyHeightCm) {
    return baseRange; // По умолчанию без модификатора
  }

  const sizeClass = getBodySizeClass(bodyHeightCm);
