...
export function createTrainingNPCSprite(
  ...
  // === ИСПРАВЛЕНО: Используем позицию из npcData если она есть ===
  // Сервер генерирует правильные позиции, клиент должен их использовать
  let x: number;
  let y: number;
  
  if (npcData.position) {
    // Позиция из серверного ответа (формат TempNPC)
    x = npcData.position.x;
    y = npcData.position.y;
    console.log(`[TrainingNPC] Using server position for "${npcData.name}": (${x}, ${y})`);
  } else if (npcData.x !== undefined && npcData.y !== undefined) {
    // Альтернативный формат (плоский)
    x = npcData.x;
    y = npcData.y;
    console.log(`[TrainingNPC] Using flat position for "${npcData.name}": (${x}, ${y})`);
  } else {
    // Fallback: случайная позиция вокруг центра
    ...
    console.log(`[TrainingNPC] Using random position for "${npcData.name}": (${x}, ${y})`);
  }
