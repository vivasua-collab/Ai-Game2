...
// Маппинг слотов экипировки на читаемые названия
...
// Интерфейс для загруженных данных техник
...
// Интерфейс для загруженных данных предметов
interface ItemData {
  id: string;
  name: string;
  type: string;
...
export function NPCViewerPanel({ npcs, loading, onLoad }: NPCViewerPanelProps) {
  ...
  // Кэш загруженных данных техник и предметов
  const [techniquesCache, setTechniquesCache] = useState<Map<string, TechniqueData>>(new Map());
  const [itemsCache, setItemsCache] = useState<Map<string, ItemData>>(new Map());
  ...
  // ✅ S3-1: Загрузка полных данных при выборе NPC (после объявления selectedNPC)
  useEffect(() => {
    ...
    const loadDetails = async () => {
      ...
      try {
        // Загружаем данные техник с функциональным обновлением
        if (selectedNPC.techniques.length > 0) {
          try {
        ...
        // Загружаем данные предметов с функциональным обновлением
        const allItemIds = [
          ...Object.values(selectedNPC.equipment).filter(Boolean) as string[],
          ...selectedNPC.inventory.map(i => i.id)
        ];
    ...
    loadDetails();
  }, [selectedNPC?.id]); // ✅ Зависимость корректна, без eslint-disable
        ...
        {selectedNPC ? (
          <ScrollArea className="flex-1">
            <div className="space-y-4 pr-4">
              ...
              {selectedNPC.techniques.length > 0 && (
                <div className="bg-slate-700/50 rounded-lg p-3 space-y-2">
                  <h4 className="text-sm font-medium text-amber-400">
                    ...
                    {loadingDetails && <Loader2 className="w-3 h-3 animate-spin inline ml-2" />}
                  </h4>
                  <div className="flex flex-wrap gap-1">
                    {selectedNPC.techniques.map((techId, i) => {
                      const techData = techniquesCache.get(techId);
                      return (
                        <Badge key={i} variant="outline" className="border-cyan-500 text-cyan-400 text-xs" title={techId}>
                          {techData?.name || techId}
                          {techData?.level && <span className="ml-1 opacity-60">Ур.{techData.level}</span>}
                        </Badge>
                      );
                    })}
                  </div>
              ...
              {/* Экипировка - НОВЫЙ БЛОК */}
              {Object.entries(selectedNPC.equipment).some(([_, v]) => v) && (
                <div className="bg-slate-700/50 rounded-lg p-3 space-y-2">
                  <h4 className="text-sm font-medium text-amber-400 flex items-center gap-2">
                    <Shield className="w-4 h-4" />
                    Экипировка
                    {loadingDetails && <Loader2 className="w-3 h-3 animate-spin" />}
                  </h4>
                  <div className="space-y-1">
                    {Object.entries(selectedNPC.equipment).map(([slot, itemId]) => {
                      if (!itemId) return null;
                      const itemData = itemsCache.get(itemId);
                      const slotIcon = slot.includes('weapon') ? <Sword className="w-3 h-3" /> :
                                       slot.includes('armor') || slot.includes('torso') || slot.includes('head') ? <Shirt className="w-3 h-3" /> :
                                       slot.includes('accessory') ? <Gem className="w-3 h-3" /> :
                      ...
                      return (
                        <div key={slot} className="flex justify-between text-sm">
                          ...
                          <span className="text-slate-300" title={itemId}>
              ...
              {selectedNPC.inventory.length > 0 && (
                <div className="bg-slate-700/50 rounded-lg p-3 space-y-2">
                  <h4 className="text-sm font-medium text-amber-400">
                    Инвентарь ({selectedNPC.inventory.length})
                    {loadingDetails && <Loader2 className="w-3 h-3 animate-spin inline ml-2" />}
                  </h4>
                  <div className="space-y-1 max-h-32 overflow-y-auto">
                    {selectedNPC.inventory.map((item, i) => {
                      const itemData = itemsCache.get(item.id);
                      return (
                        <div key={i} className="flex justify-between text-sm">
                          <span className="text-slate-300" title={item.id}>
