...
// Реэкспорт seededRandom из base-item-generator для совместимости
export { seededRandom } from './base-item-generator';
import { seededRandom } from './base-item-generator';

// Импортируем randomInRange из lore-formulas (поддерживает Range объекты)
import { randomInRange as randomInRangeUtil } from './lore-formulas';

/**
 * Случайное число в диапазоне (обёртка над lore-formulas.randomInRange)
 */
function randomInRange(range: Range, rng: () => number): number {
  return randomInRangeUtil(range, rng);
}
...
function generateCultivation(
  ...
  // Минимальный уровень культивации = 1 (смертные пока не реализованы)
  ...
  // Валидация: уровень от 1 до 9
  level = Math.max(MIN_LEVEL, Math.min(MAX_LEVEL, level));
  
  // Ограничиваем уровнем вида (но не ниже MIN_LEVEL)
  level = Math.max(MIN_LEVEL, Math.min(level, species.cultivation.maxCultivationLevel));
...
/**
 * Выбор техник по уровню культивации
 * 
 * Формула слотов: 3 + (cultivationLevel - 1)
 * - L1 = 3 техники
 * - L3 = 5 техник
 * - L9 = 11 техник
 * 
...
function selectTechniques(
  ...
  // 1. Рассчитываем максимальное количество слотов по уровню
  const maxSlots = calculateTechniqueSlots(cultivationLevel);
  
  // 2. Гарантированные техники (добавляются всегда)
  if (role.techniques?.guaranteed) {
    for (const techId of role.techniques.guaranteed) {
      if (techniques.length < maxSlots) {
        techniques.push(techId);
      }
    }
  }
  
  // 3. Возможные техники (случайный выбор до заполнения слотов)
  if (role.techniques?.possible) {
    const remaining = maxSlots - techniques.length;
    if (remaining > 0) {
  ...
  // 4. Если техник меньше чем слотов (мало possible), логируем
  if (techniques.length < maxSlots) {
    console.debug(`[NPCGenerator] NPC L${cultivationLevel} has ${techniques.length}/${maxSlots} techniques (role: ${role.id})`);
  }
