...
export function calculateFatigueFromAction(
  ...
  let newPhysical = character.fatigue + physicalChange; // В БД fatigue - это общая, разделим позже
  let newMental = (character as Character & { mentalFatigue?: number }).mentalFatigue || 0;
  newMental += mentalChange;
  ...
  return {
    ...
    mentalChange: newMental - ((character as Character & { mentalFatigue?: number }).mentalFatigue || 0),
    warnings,
    canPerform,
  };
}
...
export function calculateRestRecovery(
  ...
  return {
    ...
    mentalRecovered: Math.min(mentalRecovered, (character as Character & { mentalFatigue?: number }).mentalFatigue || 0),
  };
}
...
export function calculatePassiveRecovery(
  ...
  return {
    ...
    mentalRecovered: Math.min(passiveMentalRate * deltaTimeMinutes, (character as Character & { mentalFatigue?: number }).mentalFatigue || 0),
  };
}
