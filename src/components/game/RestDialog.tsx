...
export function RestDialog({ open, onOpenChange }: RestDialogProps) {
  ...
  const [duration, setDuration] = useState<number>(TIME_CONSTANTS.MIN_MEDITATION_TICKS);
    ...
    try {
      if (activityType === 'meditation') {
        ...
        if (data.success) {
          ...
          await loadState();
          await loadTechniques(); // Обновляем техники после медитации
        } else {
        ...
        if (data.success) {
          setResult({ message: data.message });
          await loadState();
          await loadTechniques(); // Обновляем техники после отдыха
        } else {
          setResult({ message: data.error || 'Ошибка' });
        }
      }
  ...
  }, [character, duration, activityType, meditationType, isActing, canMeditateNow, loadState, loadTechniques, conductivityProgress]);
