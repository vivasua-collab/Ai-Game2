...
interface AccessoryGeneratorPanelProps {
  onGenerate: (accessories: GeneratedEquipmentV2[]) => void;
  loading?: boolean;
}
...
// Маппинг grade -> бонусы
const GRADE_INFO: Record<string, { bonusCount: [number, number]; multiplier: number }> = {
  damaged: { bonusCount: [0, 0], multiplier: 0.7 },
  common: { bonusCount: [0, 1], multiplier: 1.0 },
  refined: { bonusCount: [1, 2], multiplier: 1.3 },
  perfect: { bonusCount: [2, 3], multiplier: 1.6 },
  transcendent: { bonusCount: [3, 4], multiplier: 2.0 },
};
...
export function AccessoryGeneratorPanel({ onGenerate, loading = false }: AccessoryGeneratorPanelProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const allAccessories: GeneratedEquipmentV2[] = [];
      ...
      // Загружаем все сгенерированные аксессуары
      const listResponse = await fetch('/api/generator/equipment?action=list&type=accessory');
      const listData = await listResponse.json();
      
      if (listData.success && listData.items) {
        const recentItems = listData.items.slice(-genCount);
        onGenerate(recentItems);
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Неизвестная ошибка');
      console.error('[AccessoryGenerator] Error:', err);
    } finally {
  ...
  const gradeData = selectedGrade !== 'random' ? GRADE_INFO[selectedGrade] : null;

  return (
    <div className="space-y-6">
      {/* Информация об аксессуарах V2 */}
      <div className="bg-slate-800/50 rounded-lg p-4 space-y-4">
        ...
        <div className="bg-slate-700/30 rounded-lg p-3 space-y-2">
          <p className="text-sm text-slate-400">
            Аксессуары — кольца, амулеты, браслеты и серьги. 
            Генератор V2 использует архитектуру "Матрёшка" с материалами и грейдами.
          </p>
          <div className="grid grid-cols-3 gap-2 text-sm">
            <div>
              <div className="text-slate-400">Типы</div>
              <div className="text-amber-400">Кольцо, Амулет, Браслет, Серьга</div>
            </div>
            <div>
              <div className="text-slate-400">Бонусы</div>
              <div className="text-cyan-400">0-4 (по грейду)</div>
            </div>
            <div>
        ...
        <div className="bg-slate-700/30 rounded-lg p-3">
          ...
          <div className="text-sm text-slate-300 font-mono bg-slate-800 p-2 rounded">
            ...
            Проводимость Ци = Material × Grade
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-800/50 rounded-lg p-4 space-y-4">
        ...
        {/* Информация о выбранном грейде */}
        {selectedGrade !== 'random' && gradeData && (
          <div className="bg-slate-700/30 rounded-lg p-3">
            <div className="text-sm text-slate-400">
              Множитель: <span className="text-amber-400">×{gradeData.multiplier}</span>
              {' • '}Бонусов: <span className="text-cyan-400">
                {gradeData.bonusCount[0]}-{gradeData.bonusCount[1]}
              </span>
            </div>
          </div>
        )}
      ...
      >
        ...
        Сгенерировать {genCount} аксессуаров (V2)
      </Button>
