...
import {
  generatedObjectsLoader,
  type GeneratedTechnique,
} from '@/lib/generator/generated-objects-loader';
import {
  getBaseCapacity,
...
export async function generateTechniquePool(options: {
  ...
  try {
    ...
    if (techniques.length === 0) {
      techniques = await generateFallbackTechniques(targetLevel, count, preferredType, preferredElement);
 ...
 */
async function generateFallbackTechniques(
  targetLevel: number,
  count: number,
  preferredType?: TechniqueType,
  preferredElement?: TechniqueElement
): Promise<Technique[]> {
  // Сначала пытаемся загрузить сгенерированные техники
  const generatedTechniques = await generatedObjectsLoader.getTechniques({
    level: targetLevel,
    type: preferredType,
  });

  if (generatedTechniques.length > 0) {
    // Фильтруем по элементу если указан
    let filtered = generatedTechniques;
    if (preferredElement) {
      filtered = generatedTechniques.filter(t => t.element === preferredElement);
    }

    // Если есть сгенерированные техники - используем их
    if (filtered.length >= count) {
      return filtered.slice(0, count).map(gt => generatedToTechnique(gt));
    }

    // Если не хватает - дополняем из пресетов
    const fromPresets = selectAndConvertPresets(
      ALL_TECHNIQUE_PRESETS,
      count - filtered.length,
      preferredType,
      preferredElement
    );
    return [...filtered.map(gt => generatedToTechnique(gt)), ...fromPresets];
  }

  // Fallback на статические пресеты
  const availablePresets = ALL_TECHNIQUE_PRESETS.filter(preset => {
    const reqLevel = preset.requirements?.cultivationLevel || 1;
    if (reqLevel > targetLevel + 1) return false;
    if (preset.level > targetLevel + 2) return false;
    return true;
  });
...
/**
 * Конвертировать сгенерированную технику в формат игры
 */
function generatedToTechnique(gt: GeneratedTechnique): Technique {
  // Рассчитываем базовую ёмкость из типа и подтипа
  const baseCapacity = gt.baseCapacity ?? getBaseCapacity(
    gt.type as CapacityTechniqueType,
    gt.subtype as CombatSubtype | undefined
  );
  ...
  return {
    ...
    type: gt.type as TechniqueType,
    subtype: gt.subtype, // ВАЖНО: сохраняем подтип (melee_strike, melee_weapon, etc.)
    element: gt.element as TechniqueElement,
    rarity: gt.rarity as TechniqueRarity,
    grade: gt.grade, // НОВАЯ СИСТЕМА GRADE (Матрёшка)
    level: gt.level,
    minCultivationLevel: Math.max(1, gt.level - 1),
    qiCost: gt.computed.finalQiCost,
    fatigueCost: {
      physical: Math.round(gt.computed.finalQiCost * 0.1),
      mental: Math.round(gt.computed.finalQiCost * 0.05),
    },
    ...
    // Копируем дополнительные поля combat техник
    weaponType: gt.weaponType,
    damageFalloff: gt.damageFalloff,
    isRangedQi: gt.isRangedQi,
    masteryProgress: 0,
    masteryBonus: 0.3,
    source: 'preset',
    createdAt: new Date(),
...
function presetToTechnique(preset: TechniquePreset): Technique {
  // Рассчитываем базовую ёмкость: сначала берём из пресета, иначе рассчитываем
  const combatSubtype = preset.effects?.combatType as CombatSubtype | undefined;
  const baseCapacity = preset.baseCapacity ?? getBaseCapacity(
    preset.techniqueType as CapacityTechniqueType,
    combatSubtype
  );
  ...
  try {
    ...
    const createdTechnique = await db.technique.create({
      data: {
        ...
        type: technique.type,
        subtype: technique.subtype || null, // Сохраняем подтип (melee_strike, melee_weapon, etc.)
        ...
        weaponType: technique.weaponType || null,
        isRangedQi: technique.isRangedQi || false,
        damageFalloff: technique.damageFalloff ? JSON.stringify(technique.damageFalloff) : null,
        source: 'created',
      },
    });
