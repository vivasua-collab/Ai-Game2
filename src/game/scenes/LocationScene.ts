...
// Tick Timer
import { tickTimer, type TickEventDetail } from '@/lib/tick-timer';
import { useTimeStore, type GameTime, type TickSpeedId } from '@/stores/time.store';
// Time Scaling
...
const BASE_PLAYER_SPEED = 200;  // Base speed at normal time (pixels per second)
const BASE_NPC_SPEED = 100;     // Base NPC speed at normal time (pixels per second)
...
export class LocationScene extends BaseScene {
  ...
  // === Tick Timer Event Handlers (for cleanup) ===
  private boundGameTick: ((event: Event) => void) | null = null;
  private boundTimerPause: ((event: Event) => void) | null = null;
  private boundTimerResume: ((event: Event) => void) | null = null;
  private boundTimerStart: ((event: Event) => void) | null = null;  // FIX: Added for timer:start

  // Current game time (updated by tick timer)
  // Phase 8.1: Updated to full GameTime interface with year/month/season
  ...
  // Phase 8.2: Track pause state for movement lock
  // FIX: Initialize from store state, not hardcoded false
  private isGamePaused: boolean = true;  // Default to paused, sync with store in create()
  ...
  update(time: number, delta: number): void {
    ...
    // === Синхронизация позиции игрока с серверным AI ===
    if (this.aiPollingClient && this.aiPollingClient.isActive()) {
  ...
  private setupAI(): void {
    ...
    this.events.on('combat:initiate', () => {
      console.log('[LocationScene] Combat initiated - switching to combat time');
      startCombat();
    });
    
    // Listen for NPC death (combat end check)
    this.events.on('npc:death', () => {
      // Check if any hostile NPCs remain
      const hasHostileNPCs = Array.from(this.npcs.values()).some(npc => npc.disposition < 0);
      if (!hasHostileNPCs && activityManager.isInCombat()) {
        console.log('[LocationScene] All hostile NPCs defeated - ending combat time');
        endCombat();
      }
    });
  ...
  private handleServerNPCAction(event: Event): void {
    const data = (event as CustomEvent<{ npcId: string; action: { type: string; target?: { x: number; y: number }; params?: Record<string, unknown> } }>).detail;
    const { npcId, action } = data;

    console.log(`[LocationScene] handleServerNPCAction: npcId=${npcId}, action=${JSON.stringify(action)}`);

    const npcSprite = this.npcPhysicsSprites.get(npcId);
    if (!npcSprite) {
      console.warn(`[LocationScene] NPC ${npcId} not found for server action. Available NPCs: ${Array.from(this.npcPhysicsSprites.keys()).slice(0, 5).join(', ')}...`);
      return;
    }

    console.log(`[LocationScene] Found NPC "${npcSprite.npcName}" at (${npcSprite.x.toFixed(0)}, ${npcSprite.y.toFixed(0)})`);

    // Выполняем действие на клиенте (только визуал!)
    npcSprite.executeServerAction(action);

    console.log(`[LocationScene] Executed action: ${action.type}`);
  }
  ...
  /**
   * Setup tick timer event listeners
   * 
   * IMPORTANT: All events come via window.dispatchEvent!
   * This ensures React UI and Phaser receive the same events.
   */
  private setupTickTimer(): void {
    ...
    this.boundTimerPause = (() => {
      this.onTimerPause();
    }) as EventListener;

    this.boundTimerResume = (() => {
      this.onTimerResume();
    }) as EventListener;

    // FIX: Add timer:start handler - was missing!
    ...
    window.addEventListener('timer:pause', this.boundTimerPause);
    window.addEventListener('timer:resume', this.boundTimerResume);
    window.addEventListener('timer:start', this.boundTimerStart);  // FIX: Now listening to timer:start

    console.log('[LocationScene] Tick timer listeners registered (including timer:start)');
  }
  ...
  private onGameTick(detail: TickEventDetail): void {
    ...
    // Emit internal event for other game systems
    this.events.emit('time-updated', detail.gameTime);

    // Log for debugging (can be removed in production)
    // console.log(`[LocationScene] Tick #${detail.tickCount} | Day ${detail.gameTime.day}, ${detail.gameTime.hour}:${detail.gameTime.minute}`);
  }
  ...
  private onTimerPause(): void {
    console.log('[LocationScene] Timer paused - pausing physics/tweens');
    
    // Phase 8.2: Set pause flag for movement lock
  ...
  private onTimerResume(): void {
    console.log('[LocationScene] Timer resumed - resuming physics/tweens');
    
    // Phase 8.2: Clear pause flag
  ...
  /**
   * FIX: Handle timer start event
   * This was missing! When tickTimer.start() is called, Phaser needs to:
  ...
  /**
   * FIX: Sync pause state and time from time.store
   * Called once during create() to ensure Phaser matches React state
   */
  private syncWithTimeStore(): void {
    const timeStore = useTimeStore.getState();

    // === FIX: AUTO-START GAME FOR NPC MOVEMENT ===
    // NPCs need physics to move, so auto-start the game if it's not running
    // This ensures NPCs can chase/attack immediately after scene load
    if (!timeStore.isRunning) {
      console.log('[LocationScene] Auto-starting game timer for NPC movement');
      tickTimer.start();
      // Update local reference after starting
      this.isGamePaused = false;
    } else {
      // Sync pause state (game is paused if explicitly paused OR timer not running)
      this.isGamePaused = timeStore.isPaused;
    }

    // Sync game time from store (no hardcoded values!)
    this.currentGameTime = { ...timeStore.gameTime };

    console.log(
      `[LocationScene] Synced with time.store: paused=${this.isGamePaused}, ` +
      `time=${timeStore.gameTime.hour}:${String(timeStore.gameTime.minute).padStart(2, '0')}`
    );

    // If game was already running and not paused, ensure physics are active
   ...
   * 
   * FIX: Speed is now scaled based on time speed setting.
   */
  private moveNPCTowards(npc: LocationNPC, targetX: number, targetY: number, baseSpeed: number): void {
    const sprite = this.npcPhysicsSprites.get(npc.id);
    if (!sprite) return;
    
    // Scale speed based on current time speed
    // FIX: Use INVERSE scaling - slow time = slow movement, fast time = fast movement
    const currentSpeed = useTimeStore.getState().speed;
    const scaledSpeed = scaleMovementSpeedInverse(baseSpeed, currentSpeed);
    
    // Используем физическое движение через setVelocity
    sprite.moveTo(targetX, targetY, scaledSpeed);
   ...
   * 
   * Phase 8.2: Added pause check - movement blocked when game is paused.
   * FIX: Auto-start/resume timer when player presses movement keys.
   */
  private handleMovement(): void {
    ...
    // FIX: Auto-start/resume when player starts moving
    // Check the actual store state, not just the local flag
    const timeStore = useTimeStore.getState();
    const isTimerStopped = !timeStore.isRunning;
    const isTimerPaused = timeStore.isPaused;
    
    if (isMoving && (isTimerStopped || isTimerPaused)) {
      console.log(`[LocationScene] Auto-${isTimerStopped ? 'start' : 'resume'} on movement input`);
      
      if (isTimerStopped) {
        // Timer was never started - start it fresh
        tickTimer.start();
      } else {
      ...
      // CRITICAL FIX: Immediately clear local pause flag
      // Events are async, but we need to allow movement NOW
      this.isGamePaused = false;
    }
    
    // Phase 8.2: Block movement when game is paused
    // FIX: Re-read store state after potential start/resume
    const isPausedInStore = useTimeStore.getState().isPaused;
    if (isPausedInStore || this.isGamePaused) {
      // Ensure velocity is zero when paused
      this.playerPhysicsBody.setVelocity(0, 0);
      return;
    }
    ...
    // === Time-scaled movement speed ===
    // Speed varies based on time speed setting
    // FIX: Use INVERSE scaling - slow time = slow movement, fast time = fast movement
    const currentSpeed = useTimeStore.getState().speed;
    const scaledSpeed = scaleMovementSpeedInverse(BASE_PLAYER_SPEED, currentSpeed);
    
    // DEBUG: Log speed changes (remove in production)
    if (isMoving) {
      console.log(`[LocationScene] Speed: ${currentSpeed} | Base: ${BASE_PLAYER_SPEED} | Scaled: ${scaledSpeed.toFixed(1)}`);
    }

    // === Устанавливаем скорость через Arcade Physics ===
    // Arcade Physics автоматически обрабатывает коллизии
    this.playerPhysicsBody.setVelocity(vx * scaledSpeed, vy * scaledSpeed);
  ...
  shutdown(): void {
    ...
    // === Удалить слушатели tick timer ===
    if (this.boundGameTick) {
      window.removeEventListener('game:tick', this.boundGameTick);
      this.boundGameTick = null;
    }
    if (this.boundTimerPause) {
      window.removeEventListener('timer:pause', this.boundTimerPause);
      this.boundTimerPause = null;
    }
    if (this.boundTimerResume) {
      window.removeEventListener('timer:resume', this.boundTimerResume);
      this.boundTimerResume = null;
    }
    // FIX: Cleanup timer:start listener
    if (this.boundTimerStart) {
      window.removeEventListener('timer:start', this.boundTimerStart);
      this.boundTimerStart = null;
    }
