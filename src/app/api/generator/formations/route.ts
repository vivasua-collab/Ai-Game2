...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (action === 'list') {
      // Загрузка формаций из хранилища (по типам)
      const formations = await presetStorage.loadFormations();
    ...
    if (action === 'stats') {
      ...
      for (const form of formations) {
        const f = form as { level: number; type: string };
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Очистка формаций
    if (action === 'clear') {
      const result = await presetStorage.clearFormations();

      return NextResponse.json({
        success: true,
        deletedObjects: result.deletedObjects,
        message: `Удалено ${result.deletedObjects} формаций`,
      });
    }
    ...
    if (mode === 'append') {
      const existing = await presetStorage.loadFormations();
      allFormations.push(...existing);
      // Обновляем счётчик
      const maxId = existing.reduce((max, f) => {
        const idNum = parseInt((f as { id: string }).id.split('_')[1]) || 0;
    ...
    // Сохраняем (с группировкой по типам)
    await presetStorage.saveFormations(allFormations, mode);
