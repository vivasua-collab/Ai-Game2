...
export function GameContainer({
  ...
  // Wait for sessionId before initializing
  useEffect(() => {
    ...
    const initGame = async () => {
      try {
        ...
        // Register with bridge - IMPORTANT: set sessionId FIRST before scenes load
        const bridge = GameBridge.getInstance();
        bridge.setGame(game as unknown as PhaserGame);
        
        // Set sessionId BEFORE scenes start - this is critical
        bridge.setSessionId(sessionId);
        console.log('[GameContainer] SessionId set on bridge:', sessionId);
  ...
  }, [sessionId]); // Re-run when sessionId changes, but initRef prevents double init

  // Update session ID separately (only after game is initialized)
  useEffect(() => {
    if (sessionId && typeof window !== 'undefined' && gameRef.current) {
      // === КРИТИЧЕСКИ ВАЖНО: Сбрасываем AI polling client при смене sessionId ===
      // Без этого polling может использовать старый sessionId в интервалах!
      resetAIPollingClient();
      console.log('[GameContainer] AIPollingClient reset for new session');
      
      GameBridge.getInstance().setSessionId(sessionId);
      console.log('[GameContainer] SessionId updated on bridge:', sessionId);
    }
  }, [sessionId]);
