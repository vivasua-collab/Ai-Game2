...
// FIX: Import tickTimer and time store for auto-unpause and time-scaling
...
// === Time System ===
let globalIsGamePaused: boolean = true;  // Synced from time.store
...
const GameSceneConfig = {
  ...
  create(this: Phaser.Scene) {
    ...
    // Sync with time.store at scene start
    const timeStore = useTimeStore.getState();
    globalIsGamePaused = timeStore.isPaused || !timeStore.isRunning;
    console.log(`[TrainingGround] Synced with time.store: paused=${globalIsGamePaused}`);
    
    // Timer event listeners for pause/resume
    const handleTimerPause = () => {
      console.log('[TrainingGround] Timer paused - pausing physics');
      globalIsGamePaused = true;
      // Проверяем, что сцена и physics существуют
    ...
    const handleTimerResume = () => {
      console.log('[TrainingGround] Timer resumed - resuming physics');
    ...
    const handleTimerStart = () => {
      console.log('[TrainingGround] Timer started - resuming physics');
      globalIsGamePaused = false;
      // Проверяем, что сцена и physics существуют
      if (scene.physics?.world) {
        scene.physics.resume();
    ...
    console.log('[TrainingGround] Timer event listeners registered');
  },
  ...
  update(this: Phaser.Scene) {
    ...
    if (cursors.left.isDown || wasd.left.isDown) velocityX = -1;
    else if (cursors.right.isDown || wasd.right.isDown) velocityX = 1;

    if (cursors.up.isDown || wasd.up.isDown) velocityY = -1;
    else if (cursors.down.isDown || wasd.down.isDown) velocityY = 1;

    const isMoving = velocityX !== 0 || velocityY !== 0;

    // === AUTO-UNPAUSE ON MOVEMENT ===
    // FIX: Auto-start/resume timer when player starts moving
    const timeStore = useTimeStore.getState();
    const isTimerStopped = !timeStore.isRunning;
    const isTimerPaused = timeStore.isPaused;

    if (isMoving && (isTimerStopped || isTimerPaused)) {
      console.log(`[TrainingGround] Auto-${isTimerStopped ? 'start' : 'resume'} on movement input`);
      ...
      // Immediately clear local pause flag for smooth movement
      globalIsGamePaused = false;
    }

    // === PAUSE CHECK ===
    // Block movement when game is paused
    const isPausedInStore = useTimeStore.getState().isPaused;
    if (isPausedInStore || globalIsGamePaused) {
      player.setVelocity(0, 0);
      return;
    }
    ...
    const scaledSpeed = scaleMovementSpeedInverse(PLAYER_SPEED, currentSpeed);
    ...
    player.setVelocity(velocityX * scaledSpeed, velocityY * scaledSpeed);
...
export function PhaserGame() {
  ...
  useEffect(() => {
    ...
    const checkAndLoad = () => {
      ...
      if (globalGameScene && !npcsLoadedRef.current) {
        ...
        // === ЗАПУСК AI POLLING ===
        // После загрузки NPC запускаем polling для серверного AI
        const aiClient = getAIPollingClient();
        aiClient.initialize(sessionId);
        aiClient.setLocationId('training_ground');
        aiClient.start();
        console.log('[TrainingNPC] AI Polling started for session:', sessionId);
  ...
  // FIX: Listen to game:tick events directly from TickTimer
  // This ensures Phaser UI updates immediately when TickTimer fires
  useEffect(() => {
