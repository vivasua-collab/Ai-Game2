...
export async function generateGameResponse(
  ...
  } else if (parsedCommand.type === "world") {
    // Глобальный запрос к миру -- команда
    enhancedSystemPrompt += `\n\n=== ГЛОБАЛЬНЫЙ ЗАПРОС ===\nЭто мета-вопрос от игрока к системе мира. Ответь прямо и информативно.
Если вопрос о времени - укажи точное время и сколько заняло действие.
Если вопрос о состоянии - опиши текущее состояние персонажа и мира.
Формат ответа:
{
  "type": "system",
  "content": "Текст ответа",
  "timeAdvance": { "minutes": X } (если нужно продвинуть время)
}`;
  } else if (parsedCommand.type === "player") {
    // Команда для ГГ !!
    enhancedSystemPrompt += `\n\n=== КОМАНДА ПЕРСОНАЖА ===\nИгрок напрямую указывает действие своего персонажа. Выполни это действие и опиши результат. Обязательно укажи сколько времени заняло действие в поле timeAdvance.`;
  }
  ...
  try {
    ...
    if (jsonMatch) {
      ...
      if (parsed.content && typeof parsed.content === "string") {
        ...
        let qiDelta: GameResponse['qiDelta'] = undefined;
        ...
        let fatigueDelta: GameResponse['fatigueDelta'] = undefined;
        ...
        let timeAdvance: GameResponse['timeAdvance'] = undefined;
