...
export const COMBAT_EVENT_TYPES = {
  ...
  // TempNPC events
  TEMP_NPC_ATTACK: 'combat:temp_npc_attack',
  TEMP_NPC_DAMAGED: 'combat:temp_npc_damaged',
  TEMP_NPC_DEATH: 'combat:temp_npc_death',
} as const;
...
export const AI_EVENT_TYPES = {
  ...
  NPC_STATE_CHANGE: 'npc:state_change',
  NPC_AI_TICK: 'npc_ai:tick',
} as const;
...
/**
 * Типы ивентов тела (Kenshi-style)
 */
export const BODY_EVENT_TYPES = {
  DAMAGE: 'body:damage',
  HEAL: 'body:heal',
  ATTACH_LIMB: 'body:attach_limb',
...
// ==================== TEMP NPC БОЕВЫЕ ИВЕНТЫ ====================

/**
 * Ивент: Атака TempNPC
 * 
 * Генерируется когда TempNPC атакует игрока.
 */
export interface TempNPCAttackEvent extends GameEventBase {
  type: typeof EVENT_TYPES.TEMP_NPC_ATTACK;
  /** ID NPC (TEMP_xxx) */
  npcId: string;
  /** ID техники NPC */
  techniqueId?: string;
...
/**
 * Ивент: Урон по TempNPC
 * 
 * Генерируется когда игрок наносит урон TempNPC.
 */
export interface TempNPCDamagedEvent extends GameEventBase {
  type: typeof EVENT_TYPES.TEMP_NPC_DAMAGED;
  /** ID NPC (TEMP_xxx) */
  npcId: string;
  /** Нанесённый урон */
  damage: number;
  ...
  /** Максимальное HP */
  maxHealth: number;
  /** Часть тела (опционально) */
  bodyPartId?: string;
}

/**
 * Ивент: Смерть TempNPC
 * 
 * Генерируется когда TempNPC умирает.
 */
export interface TempNPCDeathEvent extends GameEventBase {
  type: typeof EVENT_TYPES.TEMP_NPC_DEATH;
  /** ID NPC (TEMP_xxx) */
  npcId: string;
  /** Позиция смерти */
  position: { x: number; y: number };
 ...
 * Ивент: Движение NPC
 * 
 * Генерируется ИИ контроллером для перемещения NPC.
 */
export interface NPCMoveEvent extends GameEventBase {
  type: typeof EVENT_TYPES.NPC_MOVE;
  /** ID NPC */
  npcId: string;
  /** Целевая позиция X */
  targetX: number;
 ...
 * Ивент: Атака NPC
 * 
 * Генерируется когда NPC атакует игрока.
 */
export interface NPCAttackPlayerEvent extends GameEventBase {
  type: typeof EVENT_TYPES.NPC_ATTACK;
  /** ID NPC */
  npcId: string;
  /** ID цели (игрока) */
  targetId: string;
  /** ID техники (опционально) */
  techniqueId?: string;
  /** Урон атаки */
  damage?: number;
}
...
/**
 * Ивент: Смена состояния NPC
 * 
 * Генерируется когда NPC меняет поведение.
 */
export interface NPCStateChangeEvent extends GameEventBase {
  type: typeof EVENT_TYPES.NPC_STATE_CHANGE;
  /** ID NPC */
  npcId: string;
  /** Старое состояние */
  oldState: string;
  /** Новое состояние */
  newState: 'idle' | 'patrol' | 'chase' | 'attack' | 'flee';
}

/**
 * Ивент: Тик ИИ системы
 * 
 * Регулярный сигнал для обновления ИИ.
 */
export interface NPCAITickEvent {
  /** Время тика */
  timestamp: number;
}
...
/**
 * Ивент: Повреждение части тела
 */
export interface BodyDamageEvent extends GameEventBase {
  type: typeof EVENT_TYPES.BODY_DAMAGE;
  /** ID части тела */
  partId: string;
  /** Количество урона */
  damage: number;
...
/**
 * Ивент: Лечение части тела
 */
export interface BodyHealEvent extends GameEventBase {
  type: typeof EVENT_TYPES.BODY_HEAL;
  /** ID части тела (null = всё тело) */
  partId?: string;
  /** Количество лечения */
  amount: number;
  /** Источник лечения */
...
/**
 * Ивент: Приживление конечности
 */
export interface BodyAttachLimbEvent extends GameEventBase {
  type: typeof EVENT_TYPES.BODY_ATTACH_LIMB;
  /** Тип части тела */
  partType: string;
  /** ID части тела */
  partId: string;
  ...
  /** ID донора (опционально) */
  donorId?: string;
  /** Имя донора (опционально) */
  donorName?: string;
}

/**
 * Ивент: Регенерация конечности
 */
export interface BodyRegenerateEvent extends GameEventBase {
  type: typeof EVENT_TYPES.BODY_REGENERATE;
  /** ID части тела */
  partId: string;
  /** Использовать формацию */
  useFormation?: boolean;
}

/**
 * Ивент: Обновление состояния тела (от сервера к движку)
 */
export interface BodyUpdateEvent extends GameEventBase {
  type: typeof EVENT_TYPES.BODY_UPDATE;
  /** Сериализованное состояние тела */
  bodyState: string;
  /** Изменённые части */
  changedParts: string[];
}
...
/**
 * Проверка, является ли ивент ИИ
 */
export function isAIEvent(event: GameEvent): event is AIEvent {
  return Object.values(AI_EVENT_TYPES).includes(event.type as typeof EVENT_TYPES[keyof typeof AI_EVENT_TYPES]);
}
...
/**
 * Проверка, является ли ивент тела
 */
export function isBodyEvent(event: GameEvent): event is BodyEvent {
  return Object.values(BODY_EVENT_TYPES).includes(event.type as typeof EVENT_TYPES[keyof typeof BODY_EVENT_TYPES]);
}

// ==================== РЕЗУЛЬТАТЫ ОБРАБОТКИ ====================

/**
 * Визуальная команда для клиента
 */
...
export interface EventResult {
  ...
  /** Визуальные команды для клиента */
