...
function safeJsonParse<T>(content: string, defaultValue: T, context?: string): T {
  try {
    ...
    if (context) {
      console.warn(`[PresetStorage] JSON parse error in ${context}:`, error instanceof Error ? error.message : 'Unknown error');
    }
    return defaultValue;
  }
}
...
class IdService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      const data: IdCounters = safeJsonParse<IdCounters>(content, { counters: {} }, 'counters.json');
...
export class PresetStorageService {
  ...
  private async loadTechniquesFromPath(
    ...
    try {
      ...
      const data = safeJsonParse<{ techniques?: GeneratedTechnique[] }>(content, {}, filePath);
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          try {
            const content = await fs.readFile(fullPath, 'utf-8');
            const data = safeJsonParse<{ techniques?: unknown[]; formations?: unknown[] }>(content, {}, fullPath);
  ...
  async saveFormations(formations: unknown[], mode: 'replace' | 'append' = 'replace'): Promise<number> {
    ...
    for (const [type, typeFormations] of Object.entries(grouped)) {
      ...
      if (mode === 'append') {
        try {
          const content = await fs.readFile(filePath, 'utf-8');
          const existing = safeJsonParse<{ formations: GeneratedFormation[] }>(content, { formations: [] }, filePath);
  ...
  async clearFormations(type?: string): Promise<{ deletedFiles: number; deletedObjects: number }> {
    ...
    if (type) {
      ...
      try {
        const content = await fs.readFile(filePath, 'utf-8');
        const data = safeJsonParse<{ formations?: unknown[] }>(content, {}, filePath);
  ...
  async saveItems(
    ...
    for (const [type, typeItems] of Object.entries(grouped)) {
      ...
      if (mode === 'append') {
        try {
          const content = await fs.readFile(filePath, 'utf-8');
          const existing = safeJsonParse<{ items: unknown[] }>(content, { items: [] }, filePath);
    ...
    try {
      ...
      for (const entry of entries) {
        if (entry.isFile() && entry.name.endsWith('.json')) {
          ...
          const content = await fs.readFile(filePath, 'utf-8');
          const data = safeJsonParse<{ items?: Array<{ id: string }> }>(content, {}, filePath);
  ...
  async saveNPCs(
    ...
    for (const [type, typeNpcs] of Object.entries(grouped)) {
      ...
      if (mode === 'append') {
        try {
          const content = await fs.readFile(filePath, 'utf-8');
          const existing = safeJsonParse<{ npcs?: Array<{ id: string }> }>(content, { npcs: [] }, filePath);
    ...
    try {
      ...
      for (const entry of entries) {
        if (entry.isFile() && entry.name.endsWith('.json')) {
          ...
          const content = await fs.readFile(filePath, 'utf-8');
          const data = safeJsonParse<{ npcs?: Array<{ id: string }> }>(content, {}, filePath);
  ...
  async loadNPCsByType(type: string): Promise<Array<{ id: string; [key: string]: unknown }>> {
    ...
    try {
      const content = await fs.readFile(filePath, 'utf-8');
      const data = safeJsonParse<{ npcs?: unknown[] }>(content, {}, filePath);
      return data.npcs || [];
    } catch {
      return [];
    }
  ...
  async getManifest(): Promise<PresetManifest | null> {
    try {
      ...
      return safeJsonParse<PresetManifest | null>(content, null, 'manifest.json');
    } catch {
      return null;
    }
  }
