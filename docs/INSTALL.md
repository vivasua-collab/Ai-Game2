...
│       ├── game-shared.ts     # Общие типы (клиент/сервер)
│       └── branded.ts         # Brand types (ID)
│
├── prisma/
│   └── schema.prisma          # Схема БД (v6)
│
├── db/
...
│   ├── FUNCTIONS.md          # Перечень функций
│   ├── CODE-REVIEW.md        # Отчёт ревью
...
- **TechniquePool** - пул техник для выбора
...
3. Проверьте логи в базе данных (SystemLog)
4. Сбросьте БД и попробуйте снова

---

*Документация актуальна на 2026-02-25. Версия схемы БД: 6*
