...
import { SpriteLoader, createDirectionalSpritesheet, angleToDirectionFrame, DIRECTION_FRAMES } from '@/game/services/sprite-loader';
...
const GameSceneConfig = {
  ...
  preload(this: Phaser.Scene) {
    ...
    // Create directional spritesheet (8 directions, top-down view)
    createDirectionalSpritesheet(scene, cultivationLevel);
  ...
  create(this: Phaser.Scene) {
    ...
    // Create player with directional spritesheet
    // Frame size is 64x64, player starts facing down (frame 0 = S)
    const player = scene.physics.add.sprite(WORLD_WIDTH / 2, WORLD_HEIGHT / 2 + 100, 'player_directions');
    player.setCollideWorldBounds(true);
    player.setDepth(10);
    
    // Set initial frame (facing down)
    player.setFrame(0);
    ...
    // Mouse movement for direction (uses frame selection, not rotation)
    scene.input.on('pointermove', (pointer: Phaser.Input.Pointer) => {
      ...
      // Convert angle to direction frame (8 directions)
      // DON'T rotate the sprite - select the correct frame instead
      const frameIndex = angleToDirectionFrame(globalPlayerRotation);
  ...
  const handleMovement = useCallback(async (tilesMoved: number) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.worldTime) {
          globalWorldTime = {
            year: data.worldTime.year,
            month: data.worldTime.month,
            day: data.worldTime.day,
            hour: data.worldTime.hour,
            minute: data.worldTime.minute,
          };
        }
        
        // Обновляем Qi в глобальном персонаже (если изменилось)
        if (data.qiEffects && globalCharacter) {
          globalCharacter.currentQi = data.qiEffects.finalQi;
        }
