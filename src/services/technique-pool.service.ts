...
export async function generateTechniquePool(options: {
  ...
  try {
    ...
    await logError('TECHNIQUE_POOL', 'Failed to generate technique pool', { error });
    return {
      success: false,
...
function parseLLMResponse(content: string): Technique[] {
  try {
    ...
    return parsed.map((item, index) => ({
      ...
      maxLevel: item.maxLevel || 9,
      canEvolve: item.canEvolve !== false,
  ...
  try {
    ...
    const createdTechnique = await db.technique.create({
      data: {
        ...
        minLevel: technique.minLevel || 1,
        maxLevel: technique.maxLevel || 9,
        canEvolve: technique.canEvolve !== false,
    ...
    await logError('TECHNIQUE_POOL', 'Failed to select technique', { error });
    return {
      success: false,
