...
import { EQUIPMENT_GRADE_ORDER, type EquipmentGrade } from '@/types/grade';

/**
 * Приоритет сортировки по Grade (выше индекс - выше приоритет)
 */
const GRADE_SORT_PRIORITY: Record<string, number> = {
  damaged: 0,
  common: 1,
  refined: 2,
  perfect: 3,
...
/**
 * Получить приоритет предмета (Grade > Rarity fallback)
 */
function getItemSortPriority(item: InventoryItem): number {
  // Приоритет Grade над Rarity
  if (item.grade) {
    return GRADE_SORT_PRIORITY[item.grade] ?? 1;
  }
  return RARITY_SORT_PRIORITY[item.rarity] || 0;
}
...
function compareItems(
  ...
  switch (sortType) {
    ...
    case 'rarity':
      ...
      comparison = getItemSortPriority(b) - getItemSortPriority(a);
      break;
...
export function sortInventoryDefault(items: InventoryItem[]): InventoryItem[] {
  return [...items].sort((a, b) => {
    ...
    // 3. По редкости/грейду (высокая редкость - первые)
    const rarityDiff = getItemSortPriority(b) - getItemSortPriority(a);
