...
export interface BaseTechnique {
  ...
  baseRange: number;
  baseDuration: number;
  baseCapacity: number;  // НОВОЕ: структурная ёмкость техники
...
/**
 * Базовая ёмкость техники
 * Максимум Ци, который техника может обработать
 * Удвоение каждый уровень = 2^(level-1) × 50
 */
export const TECHNIQUE_BASE_CAPACITY: Record<number, number> = {
  1: 50, 2: 100, 3: 200, 4: 400, 5: 800,
  6: 1600, 7: 3200, 8: 6400, 9: 12800,
};

/**
 * Качество Ци по уровню культивации
 * qiDensity = 2^(cultivationLevel - 1)
 */
export const QI_DENSITY_BY_LEVEL: Record<number, number> = {
...
function generateModifiers(base: BaseTechnique, rng: () => number, rarity?: Rarity): TechniqueModifiers {
  ...
  if (tier === 1) {
    ...
    // Бонусы зависят от редкости
    const rarityIndex = ['common', 'uncommon', 'rare', 'legendary'].indexOf(rarity || 'common');
    
    if (rarityIndex >= 1) { // uncommon+
      modifiers.bonuses.damageMultiplier = 1 + (rarityIndex * 0.1);
    }
    if (rarityIndex >= 2) { // rare+
      modifiers.bonuses.critChance = rarityIndex * 5;
    }
    if (rarityIndex >= 3) { // legendary
      modifiers.bonuses.critDamage = 1.5 + (rarityIndex * 0.25);
      modifiers.bonuses.piercePercent = rarityIndex * 10;
    }
  ...
  // Количество эффектов зависит от редкости и уровня
  ...
  const maxEffects = rarity ? RARITY_MULTIPLIERS[rarity].maxEffects : 2;
...
function generateMeleeStrikeTechnique(
  ...
  let baseDamage = applyDamageVariance(
    baseValues.damage * elementMult.damage * 1.5, // УВЕЛИЧЕНО с 1.1 до 1.5
    rng,
  ...
  const baseCapacity = TECHNIQUE_BASE_CAPACITY[level] || TECHNIQUE_BASE_CAPACITY[1];
