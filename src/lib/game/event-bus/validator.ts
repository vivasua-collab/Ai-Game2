...
export function validateCombatEvent(event: Record<string, unknown>): ValidationResult {
  ...
  switch (type) {
    ...
    case EVENT_TYPES.TECHNIQUE_USE: {
      ...
      // position опционально - техника может использоваться без цели (баффы, ауры)
...
export function validateMovementEvent(event: Record<string, unknown>): ValidationResult {
  ...
  switch (type) {
    case EVENT_TYPES.MOVE: {
      // Упрощённый формат: достаточно tilesMoved или distanceMeters
      // fromPosition/toPosition опциональны (для детального логирования)
      const tilesMoved = event.tilesMoved as number | undefined;
      const distanceMeters = event.distanceMeters as number | undefined;
      
      if ((tilesMoved === undefined || tilesMoved === null) && 
          (distanceMeters === undefined || distanceMeters === null)) {
        return { valid: false, error: 'Missing tilesMoved or distanceMeters', field: 'tilesMoved' };
      }
      
      // Проверяем тип если передан
      if (tilesMoved !== undefined && typeof tilesMoved !== 'number') {
        return { valid: false, error: 'tilesMoved must be a number', field: 'tilesMoved' };
      }
      break;
    }
