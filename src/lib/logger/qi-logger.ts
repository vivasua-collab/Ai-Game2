...
// ==================== DESTABILIZATION LOGGING ====================

/**
 * Интерфейс записи лога дестабилизации
 */
export interface DestabilizationLogEntry {
  timestamp: Date;
  sessionId: string;
  characterId: string;
  techniqueId: string;
...
// Буфер логов дестабилизации
const destabilizationLogBuffer: DestabilizationLogEntry[] = [];
const MAX_DESTABILIZATION_LOG_BUFFER = 200;

/**
 * Залогировать дестабилизацию техники
 *
 * @param sessionId - ID сессии
 * @param characterId - ID персонажа
 * @param options - Параметры дестабилизации
 */
export function logDestabilization(
  sessionId: string,
  characterId: string,
  options: {
    techniqueId: string;
...
): DestabilizationLogEntry {
  const usagePercent = Math.round((options.baseQiInput / options.capacity) * 100);

  const entry: DestabilizationLogEntry = {
    timestamp: new Date(),
    sessionId,
    characterId,
    techniqueId: options.techniqueId,
    techniqueName: options.techniqueName,
    techniqueLevel: options.techniqueLevel,
    techniqueType: options.techniqueType,
    qiCost: options.qiCost,
    qiDensity: options.qiDensity,
    baseQiInput: options.baseQiInput,
    capacity: options.capacity,
    usagePercent,
    backlashDamage: options.backlashDamage,
    backlashQiLoss: options.backlashQiLoss,
    efficiency: options.efficiency,
  };

  // Добавляем в буфер
  destabilizationLogBuffer.push(entry);
  if (destabilizationLogBuffer.length > MAX_DESTABILIZATION_LOG_BUFFER) {
    destabilizationLogBuffer.shift();
  }

  // Форматированный вывод в консоль
  const time = entry.timestamp.toISOString().split('T')[1].split('.')[0];
  console.warn(
    `💥 [${time}] DESTABILIZATION! ${options.techniqueName} (L${options.techniqueLevel}) | ` +
    `Usage: ${usagePercent}% | Capacity: ${options.capacity} | ` +
    `Backlash: ${options.backlashDamage} dmg, ${options.backlashQiLoss} Qi | ` +
    `Efficiency: ${Math.round(options.efficiency * 100)}%`
  );
...
/**
 * Получить буфер логов дестабилизации
 */
export function getDestabilizationLogBuffer(): DestabilizationLogEntry[] {
  return [...destabilizationLogBuffer];
}

/**
 * Очистить буфер логов дестабилизации
 */
export function clearDestabilizationLogBuffer(): void {
  destabilizationLogBuffer.length = 0;
}

/**
 * Получить логи дестабилизации по персонажу
 */
export function getDestabilizationLogsByCharacter(characterId: string): DestabilizationLogEntry[] {
  return destabilizationLogBuffer.filter(log => log.characterId === characterId);
}

/**
 * Статистика дестабилизаций
 */
export function getDestabilizationStats(characterId?: string): {
  totalDestabilizations: number;
  totalBacklashDamage: number;
  totalQiLost: number;
  averageUsagePercent: number;
  byTechniqueType: Record<string, number>;
} {
  const logs = characterId 
    ? destabilizationLogBuffer.filter(log => log.characterId === characterId)
    : destabilizationLogBuffer;

  let totalBacklashDamage = 0;
  let totalQiLost = 0;
  let totalUsagePercent = 0;
  const byTechniqueType: Record<string, number> = {};

  for (const log of logs) {
    totalBacklashDamage += log.backlashDamage;
    totalQiLost += log.backlashQiLoss;
    totalUsagePercent += log.usagePercent;
    byTechniqueType[log.techniqueType] = (byTechniqueType[log.techniqueType] || 0) + 1;
  }

  return {
    totalDestabilizations: logs.length,
    totalBacklashDamage,
    totalQiLost,
    averageUsagePercent: logs.length > 0 ? Math.round(totalUsagePercent / logs.length) : 0,
    byTechniqueType,
  };
}
