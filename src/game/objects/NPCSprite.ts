...
export class NPCSprite extends Phaser.Physics.Arcade.Sprite {
  ...
  public executeServerAction(action: {
    ...
    switch (action.type) {
      ...
      case 'dodge':
        if (action.target && typeof action.target === 'object') {
          this.performServerDodge(action.target, action.params);
        } else {
          // Dodge на месте если нет цели (визуальный эффект)
          this.performServerDodge({ x: this.x, y: this.y - 50 }, action.params);
        }
        break;
  ...
  private performServerMove(target: { x: number; y: number }, params?: Record<string, unknown>): void {
    ...
    this.aiState = 'chase';
    console.log(`[NPCSprite:${this.npcName}] performServerMove: target=(${target.x}, ${target.y}), speed=${speed}`);
   ...
   * Серверное уклонение
   * 
   * target - точка, К КОТОРОЙ нужно dodge (перпендикулярно к угрозе)
   * Если target совпадает с текущей позицией - dodge перпендикулярно случайному направлению
   */
  private performServerDodge(target: { x: number; y: number }, params?: Record<string, unknown>): void {
    const speed = (params?.speed as number) || 300;
    
    // Вычисляем направление к target
    const dx = target.x - this.x;
    const dy = target.y - this.y;
    const distance = Math.sqrt(dx * dx + dy * dy);
    
    // Если target близко к текущей позиции - dodge в случайном направлении
    if (distance < 10) {
      const randomAngle = Math.random() * Math.PI * 2;
      this.setVelocity(Math.cos(randomAngle) * speed, Math.sin(randomAngle) * speed);
    } else {
      // Двигаемся к target
  ...
  public moveTo(targetX: number, targetY: number, speed: number): void {
    ...
    console.log(`[NPCSprite:${this.npcName}] moveTo: from (${this.x.toFixed(0)}, ${this.y.toFixed(0)}) to (${targetX}, ${targetY}), speed=${speed}, distance=${distance.toFixed(0)}, body=${this.body ? 'YES' : 'NULL'}`);
    ...
    if (distance > 5) {
      ...
      console.log(`[NPCSprite:${this.npcName}] setVelocity: (${(nx * speed).toFixed(0)}, ${(ny * speed).toFixed(0)}), body.velocity=${(this.body as Phaser.Physics.Arcade.Body)?.velocity?.x?.toFixed(0)},${(this.body as Phaser.Physics.Arcade.Body)?.velocity?.y?.toFixed(0)}`);
      
      // Обновляем направление
      const angle = Math.atan2(dy, dx) * 180 / Math.PI;
      this.setDirection(angle);
    } else {
      console.log(`[NPCSprite:${this.npcName}] Already at target, stopping`);
      this.setVelocity(0, 0);
    }
  }
