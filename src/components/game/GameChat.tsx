...
import { ELEMENT_ICONS } from "@/lib/constants/elements";
import { RARITY_TEXT_COLORS, RARITY_BORDER_COLORS } from "@/types/rarity";

// Типы боковых панелей
type PanelType = "character" | "inventory" | "techniques" | "skills" | "map" | "quests" | "relations" | null;

// RARITY_TEXT_COLORS и RARITY_BORDER_COLORS импортируются из @/types/rarity
            ...
            <div className="space-y-2">
              {inventory.map((item) => (
                <div
                  ...
                  className={`p-2 rounded-lg border border-slate-600/50 cursor-pointer hover:bg-slate-700/50 transition-colors ${RARITY_BORDER_COLORS[item.rarity || "common"]}`}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-lg">{item.icon || "📦"}</span>
                      <span className={`font-medium ${RARITY_TEXT_COLORS[item.rarity || "common"]}`}>{item.name}</span>
                    </div>
                    <div className="flex items-center gap-2">
            ...
            <div className="space-y-2">
              {techniques.map((ct) => (
                <div
                  ...
                  className={`p-2 rounded-lg border border-slate-600/50 cursor-pointer hover:bg-slate-700/50 transition-colors ${RARITY_BORDER_COLORS[ct.technique.rarity || "common"]}`}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-lg">{ELEMENT_ICONS[ct.technique.element] || "⚪"}</span>
                      <span className={`font-medium ${RARITY_TEXT_COLORS[ct.technique.rarity || "common"]}`}>
                        {ct.technique.name}
                      </span>
                    </div>
