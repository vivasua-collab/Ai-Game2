...
export class AIPollingClient {
   ...
   * 
   * ВАЖНО: Если polling уже запущен с другим sessionId,
   * сначала останавливаем его и обновляем sessionId.
   */
  ...
  /**
   * Сбросить состояние клиента (использовать при смене сессии)
   */
  reset(): void {
    this.stop();
    this.sessionId = null;
    ...
    this.log('[AIPollingClient] Reset complete');
  }
  ...
  private async performTick(): Promise<void> {
    ...
    try {
      ...
      // === КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Всегда отправляем позицию ===
      // Если lastPlayerPosition null - используем дефолтную позицию центра мира
      ...
      // === DEBUG: Логируем отправляемую позицию ===
      if (this.config.debug) {
        console.log(`[AIPollingClient] Tick sending: playerPos=(${playerPos.x}, ${playerPos.y}), locationId=${this.currentLocationId}`);
      }
  ...
  private async pollEvents(): Promise<void> {
    ...
    try {
      ...
      if (result.success && result.events && result.events.length > 0) {
        ...
        for (const broadcastEvent of result.events) {
          // === КРИТИЧНО: Преобразуем BroadcastEvent → AIEvent ===
          // Сервер отправляет: { event: 'npc:action', data: { npcId, action, ... }, ... }
          // Клиент ожидает: { type: 'npc:action', npcId: '...', data: {...}, ... }
          const aiEvent = this.transformBroadcastEvent(broadcastEvent);
          if (aiEvent) {
  ...
  /**
   * Преобразовать BroadcastEvent (от сервера) в AIEvent (для клиента)
   * 
   * BroadcastEvent: { event: 'npc:action', data: { npcId, action, ... }, locationId, timestamp }
   * AIEvent: { type: 'npc:action', npcId: '...', data: { action, ... }, timestamp }
   */
  private transformBroadcastEvent(broadcastEvent: any): AIEvent | null {
    if (!broadcastEvent || !broadcastEvent.event) {
      return null;
    }

    // Извлекаем npcId из data если есть
    const data = broadcastEvent.data || {};
    const npcId = data.npcId || '';

    // Создаём AIEvent
    const aiEvent: AIEvent = {
      type: broadcastEvent.event, // event → type
      npcId: String(npcId),
      data: data, // data остаётся как есть (action, npcState, etc.)
      timestamp: broadcastEvent.timestamp || Date.now(),
    };

    this.log(`[AIPollingClient] Transformed: ${broadcastEvent.event} → ${aiEvent.type}, npcId: ${aiEvent.npcId}`);
...
/**
 * Сбросить singleton (использовать при полной смене сессии)
 */
export function resetAIPollingClient(): void {
  ...
  console.log('[AIPollingClient] Singleton reset');
}
