    ...
    (set, get) => ({
      ...
      startGame: async (variant, customConfig, characterName) => {
        ...
        try {
          ...
          // FIX: Sync time.store from server data
      ...
      // FIX: Sync time from TickTimer - allows TickTimer to update React UI time
      setWorldTime: (time: WorldTime) => {
        set({ worldTime: time });
      },

      // FIX: Sync pause state from TickTimer
