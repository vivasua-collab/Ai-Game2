...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (data.action) {
      ...
      case 'regenerate-positions': {
        // Регенерация позиций для NPC без позиции
        const { sessionId, locationId, worldWidth, worldHeight } = data;
        
        const width = worldWidth || 1600;
        const height = worldHeight || 1200;
        ...
        for (const npc of npcs) {
          if (!npc.position || npc.position.x === undefined) {
            // Генерируем случайную позицию вокруг центра
            const angle = Math.random() * Math.PI * 2;
            const distance = 200 + Math.random() * 300;
            npc.position = {
              x: Math.round(width / 2 + Math.cos(angle) * distance),
              y: Math.round(height / 2 + Math.sin(angle) * distance),
            };
            updated++;
          }
        }
        ...
        return NextResponse.json({
          ...
          message: `Обновлены позиции ${updated} из ${npcs.length} NPC`,
        });
      }
