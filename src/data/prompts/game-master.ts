...
2. qiDelta.qiChange - это ИЗМЕНЕНИЕ Ци в ядре, а НЕ абсолютное значение!
   - Положительное число = накопление в ядре (например, +5)
...
КАК РАБОТАЕТ ПРОРЫВ:
1. Игрок накапливает Ци в ядре через медитацию (до 100%)
2. При 100% ядра Ци ПЕРЕНОСИТСЯ в накопление для прорыва (accumulatedQi)
3. Ядро опустошается, можно снова наполнять
4. Прорыв возможен когда accumulatedQi >= 10 × ёмкость_ядра

ПРИМЕР СТАРТА (ёмкость = 1000):
- Нужно накопить 10,000 accumulatedQi для прорыва
- Это = 10 полных заполнений ядра
- После прорыва: ёмкость растёт на 10%, accumulatedQi сбрасывается
...
- ПЕРЕПОЛНЕНИЕ ЯДРА: при 100% ядра медитация переносит Ци в накопление
