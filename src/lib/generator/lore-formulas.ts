 ...
 * Уровень 9 = 256 ед/см³
 */
export function getQiDensity(level: number): number {
  if (level < 1 || level > 9) {
    console.warn(`[LoreFormulas] Invalid cultivation level: ${level}`);
...
export function calculateCoreCapacity(baseVolume: number, level: number, subLevel: number): number {
  ...
  // Плотность Ци
  const density = getQiDensity(level);
...
export function debugExampleNPC(): void {
  ...
  console.log(`Плотность Ци (уровень ${level}): ${getQiDensity(level)}`);
