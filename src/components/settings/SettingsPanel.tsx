              ...
              <TabsContent value="consumable">
                <Suspense fallback={<PanelLoader />}>
                  ...
                  onGenerate={async (consumables) => {
                    setLoading(true);
                    const success = await saveItemsToServer(consumables as unknown as Array<{ id: string; type: string; [key: string]: unknown }>, 'append');
                    if (success) {
              ...
              <TabsContent value="qi_stone">
                <Suspense fallback={<PanelLoader />}>
                  ...
                  onGenerate={async (stones) => {
                    setLoading(true);
                    const success = await saveItemsToServer(stones as unknown as Array<{ id: string; type: string; [key: string]: unknown }>, 'append');
                    if (success) {
              ...
              <TabsContent value="charger">
                <Suspense fallback={<PanelLoader />}>
                  ...
                  onGenerate={async (chargers) => {
                    // V2 API уже сохраняет, просто обновляем UI
                    setLoading(false);
