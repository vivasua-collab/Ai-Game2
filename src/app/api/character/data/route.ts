/**
 * API данных персонажа (техники, навыки, базовые данные)
 * GET - получить техники, навыки и базовые данные персонажа
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    // ✅ Получаем базовые данные персонажа для UI (cultivationLevel, spiritStones)
    ...
    if (type === "techniques") {
      ...
      return NextResponse.json({ success: true, techniques, character });
    }

    if (type === "skills") {
      const skills = await getCharacterSkills(characterId);
      return NextResponse.json({ success: true, skills, character });
    }
