...
// V1 Generator (@deprecated - только типы для совместимости)
import type { GeneratedTechnique as GenTechniqueV1 } from './technique-generator';
 ...
 */
function safeJsonParse<T>(content: string, defaultValue: T, context?: string): T {
  try {
    return JSON.parse(content) as T;
  } catch (error) {
    if (context) {
      console.warn(`[GeneratedObjectsLoader] JSON parse error in ${context}:`, error instanceof Error ? error.message : 'Unknown error');
    }
    return defaultValue;
  }
}
...
export interface GeneratedTechnique {
  ...
  level: number;
  rarity: string;
  grade?: string; // V2: новая система Grade
  baseCapacity?: number | null; // V2: структурная ёмкость
  computed: {
    finalDamage: number;
    finalQiCost: number;
    finalRange: number;
...
class GeneratedObjectsLoader {
  ...
  /**
   * Автогенерация техник при отсутствии файлов (V2)
  ...
  private async autoGenerateTechniques(): Promise<GeneratedTechnique[]> {
    ...
    try {
      ...
      for (const subtype of combatSubtypes) {
        console.log(`[GeneratedObjectsLoader] Generating 10 combat/${subtype} techniques (V2)...`);
        ...
        for (let i = 0; i < 10; i++) {
          ...
          try {
            ...
            const genTechnique = this.convertV2ToGenerated(technique);
      ...
      for (const type of otherTypes) {
        console.log(`[GeneratedObjectsLoader] Generating 10 ${type} techniques (V2)...`);
      ...
      if (allTechniques.length > 0) {
        ...
        console.log(`[GeneratedObjectsLoader] Auto-generated ${allTechniques.length} techniques (V2)`);
      }
   ...
   * @deprecated Используйте convertV2ToGenerated
   */
  ...
  /**
   * Конвертировать V2 технику в формат для сохранения
   */
  private convertV2ToGenerated(technique: GeneratedTechniqueV2): GeneratedTechnique {
    ...
    return {
      ...
      element: technique.element,
      level: technique.level,
      rarity: gradeToRarity[technique.grade] || 'common',
  ...
  async loadFormations(forceReload = false): Promise<LoadedObjects<GeneratedFormation>> {
    ...
    try {
      ...
      const data = safeJsonParse<{ formations?: GeneratedFormation[] }>(content, {}, filePath);
    ...
    try {
      ...
      for (const entry of entries) {
        if (entry.isFile() && entry.name.endsWith('.json')) {
          ...
          const content = await fs.readFile(filePath, 'utf-8');
          const data = safeJsonParse<{ items?: GeneratedItem[] }>(content, {}, filePath);
  ...
  async loadObjects(type: string): Promise<GeneratedItem[]> {
    ...
    if (type === 'consumables') {
      ...
      // АВТОГЕНЕРАЦИЯ: если расходников нет - генерируем базовые
  ...
  /**
   * Автогенерация расходников при отсутствии файлов
   */
  private async autoGenerateConsumables(): Promise<GeneratedItem[]> {
    if (this.isGenerating) {
      console.log('[GeneratedObjectsLoader] Already generating consumables, waiting...');
      return [];
    }

    this.isGenerating = true;
    const allConsumables: GeneratedItem[] = [];

    try {
      console.log('[GeneratedObjectsLoader] Auto-generating consumables...');

      // Динамический импорт генератора
      const { generateConsumables } = await import('./consumable-generator');

      // Генерируем по 10 расходников каждого типа для уровня 1
      const types: Array<'pill' | 'elixir' | 'food' | 'scroll'> = ['pill', 'elixir', 'food', 'scroll'];

      for (const type of types) {
        console.log(`[GeneratedObjectsLoader] Generating 10 ${type} consumables...`);

        const result = generateConsumables(10, {
          type,
          level: 1,
          mode: 'append',
        });
        ...
        for (const consumable of result.consumables) {
          allConsumables.push(this.convertConsumableToGenerated(consumable));
        }
      }

      // Сохраняем в файл
      if (allConsumables.length > 0) {
        await this.saveAutoGeneratedConsumables(allConsumables);
        console.log(`[GeneratedObjectsLoader] Auto-generated ${allConsumables.length} consumables`);
      }

    } catch (error) {
      console.error('[GeneratedObjectsLoader] Auto-generation of consumables failed:', error);
    } finally {
      this.isGenerating = false;
    }
  ...
  /**
   * Конвертировать Consumable в GeneratedItem
   */
  private convertConsumableToGenerated(consumable: any): GeneratedItem {
    return {
      id: consumable.id,
      ...
      type: 'consumable',
      category: `consumable_${consumable.type}`,
      rarity: consumable.rarity,
      icon: 'consumable',
      sizeWidth: 1,
      sizeHeight: 1,
      effects: consumable.effect ? [{
        type: consumable.effect.type,
        value: consumable.effect.value,
        duration: consumable.effect.duration,
      }] : [],
      value: 5 * consumable.level,
      currency: 'spirit_stones',
    };
  }

  /**
   * Сохранить автосгенерированные расходники
   */
  private async saveAutoGeneratedConsumables(consumables: GeneratedItem[]): Promise<void> {
    const filePath = path.join(ITEMS_DIR, 'consumable.json');
    ...
    try {
      ...
      console.log(`[GeneratedObjectsLoader] Saved ${consumables.length} consumables to ${filePath}`);
    } catch (saveError) {
      console.error(`[GeneratedObjectsLoader] Failed to save consumables:`, saveError);
    }
  }
