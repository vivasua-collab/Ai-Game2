...
| 2 | Разделение qi-system / qi-shared | 🟡 Средний | 🟢 Низкая | ✅ Завершено |
| 3 | Zod валидация для всех API | 🔴 Высокий | 🟡 Средняя | ✅ Завершено |
| 4 | Рефакторинг request-router | 🟡 Средний | 🟡 Средняя | ✅ Завершено |
| 5 | Исправление Google Fonts | 🔴 Критичный | 🟢 Низкая | ✅ Завершено |
| 6 | Упрощённая система карты | 🔴 Высокий | 🟡 Средняя | ✅ Завершено |
| 7 | Интеграция qi-insight | 🔴 Высокий | 🟡 Средняя | ✅ Завершено |
...
- [x] Найти все дублирования `interface LocationData`
- [x] Найти типы, определённые в нескольких местах
- [x] Составить список типов для миграции
...
- [x] Создать файл `src/types/game-shared.ts`
- [x] Перенести `LocationData` интерфейс (универсальная версия)
- [x] Добавить комментарии о назначении файла
...
- [x] Обновить `src/lib/game/qi-system.ts` → `import { LocationData } from '@/types/game-shared'`
- [x] Обновить `src/lib/game/qi-shared.ts` → `import { LocationData } from '@/types/game-shared'`
- [x] Обновить `src/lib/game/request-router.ts` → `import { LocationData } from '@/types/game-shared'`
- [x] Удалить локальные определения `interface LocationData`

#### Шаг 1.4: Проверка сборки
- [x] Запустить `bun run lint`
- [x] Проверить TypeScript: `npx tsc --noEmit`
- [x] Убедиться, что нет ошибок импортов
...
- [x] Проверить, какие re-exports используются
- [x] Определить, можно ли убрать re-exports
...
- [x] Проверить все файлы, использующие эти функции
- [x] Обновить импорты на прямые из qi-shared

#### Шаг 2.3: Удаление re-exports
- [x] Удалить блок re-exports из qi-system.ts
- [x] Оставить только `performMeditation` и `attemptBreakthrough`

#### Шаг 2.4: Документирование ответственности
- [x] Обновить JSDoc в qi-system.ts с чётким описанием
- [x] Обновить JSDoc в qi-shared.ts с чётким описанием
...
- [x] `/api/cheats` — добавить валидацию
- [x] `/api/inventory` — добавить валидацию
- [x] `/api/inventory/use` — добавить валидацию
- [x] `/api/techniques/pool` — добавить валидацию
- [x] `/api/database/migrate` — добавить валидацию (если нужен body)
- [x] `/api/database/reset` — добавить валидацию
- [x] `/api/logs` — добавить валидацию query params
- [x] `/api/character/data` — добавить валидацию
...
- [x] Создать `src/lib/game/response-builders.ts`
- [x] Перенести `buildStatusResponse`, `buildTechniquesResponse`, `buildStatsResponse`, `buildLocationResponse`
...
- [x] Группировать regex паттерны по категориям
- [x] Вынести паттерны в константы
...
- [x] Обновить импорты в `request-router.ts`
- [x] Убедиться, что все экспорты работают

#### Шаг 4.4: Документирование
- [x] Добавить JSDoc для всех функций
- [x] Обновить комментарии о назначении модуля
...
### Ручные проверки
- [x] Проверить запуск игры (Create World)
- [x] Проверить отправку сообщения в чат
- [x] Проверить медитацию (накопление Ци)
- [x] Проверить прорыв
- [x] Проверить чит-команды
- [x] Проверить инвентарь
