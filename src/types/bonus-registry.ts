...
export type BonusCategory = 
  ...
  | 'qi'             // Ци-параметры (регенерация, ёмкость, проводимость)
...
export interface BonusDefinition {
  ...
  isMultiplier?: boolean;        // Является ли множителем
  ...
  levelScaling?: number;         // Альтернативное масштабирование по уровню
  maxValue?: number;             // Максимальное значение
  minValue?: number;             // Минимальное значение
  variance?: number;             // Вариация значения (случайное отклонение)
 ...
 * Источник бонуса
 */
export type BonusSource = 'material' | 'grade' | 'base' | 'enchant' | 'technique' | 'artifact';
...
export interface GeneratedBonus {
  ...
  displayText: string;
  type?: string;          // Тип бонуса (для совместимости)
  isMultiplier?: boolean; // Является ли множителем
  category?: BonusCategory; // Категория бонуса
  source?: BonusSource;     // Источник бонуса
}
...
export interface ActiveCondition {
  ...
  source: 'technique' | 'weapon' | 'artifact' | 'environment' | 'system' | 'item' | 'event';
